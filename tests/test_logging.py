...
def main():
    ...
    slog.info("query_executed", context="db.postgres",
              query="SELECT * FROM users", duration_ms=45)
    slog.error("connection_failed", context="redis.client",
               host="localhost", port=6379, error="timeout")
    print()
    ...
    slog.info("user_action",
              user_id=9999,
              action="login",
              ip="192.168.1.100",
              user_agent="Mozilla/5.0")

    slog.debug("cache_hit",
    ...
    slog.info("ai_response_received",
              context="ai.client",
              model="google/gemini-2.0-flash-001",
              tokens=420,
              duration_ms=1234,
    ...
    # Session management
    slog.debug("session_created",
               context="session.manager",
               session_key="telegram:1:123456789",
               model="google/gemini-2.0-flash-001",
               max_history=10)

    slog.debug("session_cleanup",
               context="session.manager",
               expired_count=5,
               total_sessions=42,
    ...
    print("-" * 80)
    slog.info("user_registration",
              email="user@example.com",
              name="John Doe",
              company="Tech Corp Inc.",
              role="Senior Developer")

    slog.error("validation_error",
               field="password",
