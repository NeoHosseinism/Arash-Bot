"""
OpenRouter Service Integration Tests
Tests connectivity and functionality of the OpenRouter service
"""
import asyncio
import pytest
import httpx
...
class TestOpenRouterConnectivity:
    ...
    def base_url(self):
        ...
        return settings.OPENROUTER_SERVICE_URL
    ...
    async def test_base_url_reachable(self, client, base_url):
        ...
        try:
            ...
            pytest.fail(f"✗ Cannot connect to {base_url}: {e}")
    ...
    async def test_health_endpoint(self, client, base_url):
        ...
        try:
            ...
            print(f"✓ Health endpoint OK: {response.text[:100]}")
    ...
    async def test_chat_endpoint_format(self, client, base_url):
        ...
        try:
            ...
            data = response.json()
            assert "Response" in data or "response" in data, \
...
class TestOpenRouterClient:
    """Test OpenRouter client wrapper"""
    
    @pytest.mark.asyncio
    async def test_client_initialization(self):
        """Test client is properly initialized"""
        assert openrouter_client is not None
        assert openrouter_client.base_url == settings.OPENROUTER_SERVICE_URL
        assert openrouter_client.max_retries == 3
    ...
    async def test_health_check_method(self):
        ...
        try:
            ...
            if is_healthy:
                ...
                print("⚠ Service reported unhealthy")
        except Exception as e:
    ...
    async def test_send_chat_request(self):
        ...
        try:
            ...
            print(f"✓ Chat request successful: {response}")
        except Exception as e:
...
class TestOpenRouterConfiguration:
    ...
    def test_url_configured(self):
        """Test that OpenRouter URL is configured"""
        assert settings.OPENROUTER_SERVICE_URL, \
            "OPENROUTER_SERVICE_URL not configured in .env"
        assert settings.OPENROUTER_SERVICE_URL.startswith("http"), \
            "OPENROUTER_SERVICE_URL must start with http or https"
        print(f"✓ URL configured: {settings.OPENROUTER_SERVICE_URL}")
    
    def test_url_format(self):
        """Test URL format is valid"""
        url = settings.OPENROUTER_SERVICE_URL
        assert not url.endswith("/"), \
            "OPENROUTER_SERVICE_URL should not end with /"
...
def manual_test():
    ...
    print("=" * 70)
    print("OpenRouter Service Connectivity Test".center(70))
    print("=" * 70)
    print()
    
    print(f"Service URL: {settings.OPENROUTER_SERVICE_URL}")
    print(f"Telegram Model: {settings.TELEGRAM_MODEL}")
    ...
    async def run_tests():
        ...
        base_url = settings.OPENROUTER_SERVICE_URL
        ...
        try:
            ...
            print(f"   ✗ Timeout: Service did not respond within 5 seconds")
        except Exception as e:
            print(f"   ✗ Error: {e}")
        ...
        payload = {
            ...
            "SessionId": "test_session",
            "History": [],
            "Pipeline": settings.TELEGRAM_MODEL,
            "Query": "سلام، این یک تست است",
            "AudioFile": None,
            "Files": []
        }
        ...
        try:
            ...
            print(f"   ✗ Timeout: Chat request took too long (>30s)")
        ...
        try:
            ...
            if is_healthy:
                ...
                print(f"   ⚠ Service reported unhealthy")
        except Exception as e:
            print(f"   ✗ Error: {e}")
        ...
        print()
        print("Next Steps:")
        print("  • If all tests pass: Your OpenRouter service is working!")
        print("  • If tests fail: Check the OPENROUTER_SERVICE_URL in .env")
